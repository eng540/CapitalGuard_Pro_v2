...
def test_second_offset_source_timestamp_uses_exchange_grid():
    ...
    candles = [grid_start + timedelta(minutes=i) for i in range(1440)]
    ...
    assert coverage.actual_start == grid_start
    assert coverage.actual_end == grid_start + timedelta(minutes=1439)
...
def test_real_gap_remains_gapped():
    ...
    grid_start = start.replace(second=0, microsecond=0)
    candles = [grid_start + timedelta(minutes=i) for i in range(10) if i != 5]
