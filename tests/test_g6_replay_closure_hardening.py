...
def test_target_labels_and_activity_clocks_are_not_numeric_targets():
    ...
    assert [item["price"] for item in targets] == [Decimal("72300"), Decimal("72400")]


def test_real_target_numbers_remain_unchanged():
    targets = normalize_targets("TP1: 71700 TP2: 71800")
    assert [item["price"] for item in targets] == [Decimal("71700"), Decimal("71800")]


def test_clock_safety_accepts_non_clock_price_tokens():
    assert parse_decimal_token("71700") == Decimal("71700")
    assert parse_decimal_token("1.2345") == Decimal("1.2345")
