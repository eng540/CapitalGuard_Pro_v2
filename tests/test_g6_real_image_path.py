...
async def _fake_post_with_retries(*_args, **_kwargs):
    response = {"choices": [{"message": {"content": json.dumps({"asset": "BTCUSDT", "market": "Futures", "side": "LONG", "entry": "77000", "stop_loss": "76000", "targets": [{"price": "78000", "close_percent": 100.0}], "leverage": "5"})}}]}
    return True, response, 200, json.dumps(response)


@pytest.mark.asyncio
async def test_real_png_traverses_existing_vision_adapter_and_historical_chain(tmp_path, monkeypatch, db_session):
    ...
    assert vision_result["asset"] == "BTCUSDT"
    ...
    receipt.raw_text = "BTCUSDT Futures LONG"
    receipt.metadata_json = {"media": {"media_type": "PHOTO", "file_id": "telegram-photo-file", "media_unique_id": "telegram-photo-unique"}}
    ...
    materialized = HistoricalSemanticMaterializationService().materialize_revision(db_session, revision_id=revision.id, image_result={"status": "success", "data": vision_result}, image_provenance={"media_id": "telegram-photo-unique", "file_id": "telegram-photo-file", "provider": "openai", "model": "gpt-4o-mini"})
...
class _ProxyClient:
    def __init__(self, payload, *args, **kwargs): self.payload = payload; self.calls = []
...
async def test_file_id_proxy_reaches_existing_image_service_and_semantic_chain(db_session, monkeypatch):
    ...
    payload = {"asset": "BTCUSDT", "market": "Futures", "side": "LONG", "entry": "77000", "stop_loss": "76000", "targets": [{"price": "78000", "close_percent": 100.0}], "leverage": "5"}
    clients = []
    def make_client(*args, **kwargs):
        client = _ProxyClient(payload, *args, **kwargs); clients.append(client); return client
    ...
    monkeypatch.setattr(image_parsing_service.ImageParsingService, "_get_telegram_file_url", lambda self, file_id: _async_value(f"https://telegram.test/file/btc_signal.png?file_id={file_id}"))
    ...
    receipt.metadata_json = {"media": {"media_type": "PHOTO", "file_id": "telegram-photo-file", "media_unique_id": "telegram-photo-unique"}}
    revision = HistoricalMessageFoundationService().record_receipt(db_session, receipt=receipt)
    projection = HistoricalSemanticMaterializationService().materialize_revision(db_session, revision_id=revision.id, image_result=result, image_provenance={"media_id": "telegram-photo-unique", "file_id": "telegram-photo-file"})
...
@pytest.mark.asyncio
async def test_historical_handler_helper_runs_file_id_to_semantic_materialization(db_session, monkeypatch):
    from types import SimpleNamespace
    from capitalguard.interfaces.telegram.historical_forwarding_handler import _materialize_historical_content
    _, receipt = make_reviewed_batch(db_session)
    receipt.raw_text = "BTCUSDT Futures LONG"
    receipt.metadata_json = {"media": {"media_type": "PHOTO", "file_id": "telegram-photo-file-2", "media_unique_id": "telegram-photo-unique-2"}}
    revision = HistoricalMessageFoundationService().record_receipt(db_session, receipt=receipt)
    payload = {"status": "success", "data": {"asset": "BTCUSDT", "market": "Futures", "side": "LONG", "entry": "77000", "stop_loss": "76000", "targets": [{"price": "78000", "close_percent": 100.0}], "leverage": "5"}}
    async def fake_parse(self, user_id, file_id):
        assert user_id == 99; assert file_id == "telegram-photo-file-2"; return payload
    monkeypatch.setattr("capitalguard.interfaces.telegram.historical_forwarding_handler.ImageParsingService.parse_image_from_file_id", fake_parse)
    result = await _materialize_historical_content(db_session, SimpleNamespace(id=99), receipt)
    assert result["status"] == "SUCCESS"
...
async def test_historical_handler_helper_preserves_text_image_conflict(db_session, monkeypatch):
    ...
    from capitalguard.interfaces.telegram.historical_forwarding_handler import _materialize_historical_content
    _, receipt = make_reviewed_batch(db_session)
    receipt.raw_text = "BTCUSDT Futures LONG Entry 77K SL 76K TP 78K"
    receipt.metadata_json = {"media": {"media_type": "PHOTO", "file_id": "telegram-photo-conflict", "media_unique_id": "telegram-photo-conflict-unique"}}
    image_payload = {"status": "success", "data": {"asset": "BTCUSDT", "market": "Futures", "side": "LONG", "entry": "78000", "stop_loss": "76000", "targets": [{"price": "79000", "close_percent": 100.0}], "leverage": "5"}}
    async def fake_parse(self, user_id, file_id):
        assert user_id == 99; assert file_id == "telegram-photo-conflict"; return image_payload
    monkeypatch.setattr("capitalguard.interfaces.telegram.historical_forwarding_handler.ImageParsingService.parse_image_from_file_id", fake_parse)
    result = await _materialize_historical_content(db_session, SimpleNamespace(id=99), receipt)
