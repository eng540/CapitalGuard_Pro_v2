...
from capitalguard.infrastructure.db.models import ChannelCatalog, HistoricalForwardReceipt, HistoricalRecommendationDraft, HistoricalReplayRun, HistoricalSignal, HistoricalSignalEvidence, PublicationDelivery, Recommendation, UserTrade

SOURCE_TIME = datetime(2025, 1, 1, 12, 0, tzinfo=timezone.utc)

class FakeProvider:
    def __init__(self, candles=None, error=None): self.candles=list(candles or []); self.error=error; self.calls=0
...
def test_auto_progression_recovers_reused_revision_without_receipt_binding(db_session):
    ...
    assert projection["status"]=="SUCCESS"; assert result["progressed"]==1; assert result["items"][0]["replay_status"]=="COMPLETED"
...
def test_unclaimed_source_is_replayable_without_becoming_trusted_or_live(db_session):
    ...
    assert receipt.validation_status=="INGESTED"; assert len(db_session.execute(select(HistoricalSignalEvidence)).scalars().all())==1
...
def test_repeated_successful_job_is_idempotent(db_session):
    ...
    assert first["status"]==second["status"]=="PARTIAL"; assert first["progressed"]==second["progressed"]==1; assert provider.calls==1; assert len(db_session.execute(select(HistoricalSignal)).scalars().all())==1 and len(db_session.execute(select(HistoricalReplayRun)).scalars().all())==1
...
def test_multiple_targets_are_preserved_and_partial_target_does_not_close_lifecycle(db_session):
    ...
    assert projection["status"]=="SUCCESS"; result=HistoricalForwardingService().auto_progress_canonical_batch(db_session,batch_id=batch.id,replay_end=SOURCE_TIME+timedelta(minutes=10),limit=3,provider=FakeProvider(_candles(high=106)))
...
def test_replay_window_shortfall_is_explicit_and_reaches_canonical_g6(db_session):
    old_time=datetime(2025,1,1,12,0,tzinfo=timezone.utc); batch,_,_,_=_auto_batch(db_session,raw_text="#BTCUSDT LONG Entry 100 Stop 90 TP1 110 Futures",source_time=old_time)
...
def test_auto_policy_never_sets_human_reviewer_identity(db_session):
    ...
    assert result["status"]=="COMPLETED"; assert draft.reviewed_by_user_id is None and draft.override_json["actor_type"]=="SYSTEM_POLICY" and draft.override_json["human_reviewer"] is False and draft.override_json["live_activation"] is False
