...
def test_canonical_complete_auto_progression_materializes_and_replays_without_live_entities(db_session):
    ...
    item = result["items"][0]
    assert item["status"] == "REPLAYED" and item["replay_status"] == "COMPLETED" and item["lifecycle_status"] == "CLOSED_TARGETS" and item["source_timestamp"] == SOURCE_TIME.isoformat()
...
def test_unclaimed_source_is_replayable_without_becoming_trusted_or_live(db_session):
    ...
    assert (
        ...
        and result["progressed"] == 1
        and result["failed"] == 0
        and result["partial"] == 1
    )
    ...
    evidence = db_session.execute(select(HistoricalSignalEvidence)).scalars().all()[0]
    assert evidence.metadata_json["source_trust"] == "UNVERIFIED_FORWARD"
    signal = db_session.execute(select(HistoricalSignal)).scalars().all()[0]
    assert signal.trust_tier == "UNVERIFIED" and signal.eligible_for_ranking is False
...
def test_provider_failure_preserves_evidence_and_g5_and_is_safe_to_retry(db_session):
    ...
    first = service.auto_progress_canonical_batch(
        ...
        replay_end=SOURCE_TIME + timedelta(minutes=10),
        limit=3,
        provider=FakeProvider(error=RuntimeError("provider timeout")),
    )
    assert (
        first["progressed"] == 1
        and first["failed"] == 1
        and first["partial"] == 0
        and first["items"][0]["status"] == "REPLAY_FAILED"
        and "G5 evidence was preserved" in first["items"][0]["reason"]
    )
    ...
    assert (
        second["items"][0]["status"] == "REPLAY_FAILED"
        and second["failed"] == 1
...
def test_repeated_successful_job_is_idempotent(db_session):
    ...
    service = HistoricalForwardingService()
    provider = FakeProvider(_candles())
    kwargs = {"batch_id": batch.id, "replay_end": SOURCE_TIME + timedelta(minutes=10), "limit": 3, "provider": provider}
    ...
    assert (
        ...
        and first["progressed"] == second["progressed"] == 1
        and first["failed"] == second["failed"] == 0
        and first["partial"] == second["partial"] == 1
...
def test_multiple_targets_are_preserved_and_partial_target_does_not_close_lifecycle(db_session):
    ...
    result = HistoricalForwardingService().auto_progress_canonical_batch(
        ...
        replay_end=SOURCE_TIME + timedelta(minutes=10),
        limit=3,
        provider=FakeProvider(_candles(high=106)),
    )
    assert result["progressed"] == 1
    signal = db_session.execute(select(HistoricalSignal)).scalar_one()
    assert len(signal.targets) == 2 and result["items"][0]["lifecycle_status"] == "ACTIVE_POSITION"
...
def test_ambiguous_candle_is_unverifiable_and_not_final(db_session):
    ...
    item = result["items"][0]
    assert item["replay_status"] == "COMPLETED_UNVERIFIABLE" and item["lifecycle_status"] == "CLOSED_UNVERIFIABLE" and item["status"] == "REPLAYED" and result["status"] == "COMPLETED_UNVERIFIABLE"


def test_completed_unverifiable_is_not_partial_or_failed(db_session):
...
def test_replay_partial_is_partial_not_failed(db_session):
    batch, _, _, _ = _auto_batch(
        db_session,
        raw_text="#BTCUSDT LONG Entry 100 Stop 90 TP1 110 Futures",
    )
...
def test_replay_window_shortfall_is_explicit_and_reaches_canonical_g6(db_session):
    ...
    provider = FakeProvider([
        MarketCandle(asset="BTCUSDT", market="Futures", open_time=old_time, open=100, high=101, low=99, close=100, volume=1, data_source="FAKE"),
        MarketCandle(asset="BTCUSDT", market="Futures", open_time=old_time + timedelta(minutes=15), open=100, high=101, low=99, close=100, volume=1, data_source="FAKE"),
    ])
    result = HistoricalForwardingService().auto_progress_canonical_batch(
        db_session,
        batch_id=batch.id,
    ...
    assert len(runs) == 1 and runs[0].status == "REPLAY_PARTIAL" and runs[0].coverage_status == "PARTIAL_WINDOW" and len(db_session.execute(select(HistoricalSignal)).scalars().all()) == 1
...
def test_auto_policy_never_sets_human_reviewer_identity(db_session):
    ...
    draft = db_session.execute(
        select(HistoricalRecommendationDraft).where(HistoricalRecommendationDraft.revision_id == revision.id)
    ).scalar_one()
