...
def _record(index: int) -> BinanceHistoricalCandle:
    ...
    return BinanceHistoricalCandle(
        symbol="BTCUSDT",
        market="SPOT",
        interval="1m",
        open_time=timestamp,
...
def test_provider_does_not_treat_short_page_as_completion():
    ...
    candles, _endpoint, coverage = provider.fetch_with_coverage(
        asset="BTCUSDT",
        market="SPOT",
        interval="1m",
        start=START,
        end=START + timedelta(minutes=1199),
        limit=1500,
    )
...
def test_provider_preserves_partial_window_when_source_stops_early():
    ...
    assert coverage.coverage_ratio == 500 / 1001
