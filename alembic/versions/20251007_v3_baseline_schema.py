# alembic/versions/20251007_v3_baseline_schema.py (FINAL & COMPLETE)
...
def upgrade() -> None:
    ...
    op.execute("CREATE TYPE recommendationstatusenum AS ENUM ('PENDING', 'ACTIVE', 'CLOSED')")
    op.execute("CREATE TYPE ordertypeenum AS ENUM ('MARKET', 'LIMIT', 'STOP_MARKET')")
    op.execute("CREATE TYPE exitstrategyenum AS ENUM ('CLOSE_AT_FINAL_TP', 'MANUAL_CLOSE_ONLY')")
    ...
    sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=False),
    sa.Column('closed_at', sa.DateTime(timezone=True), nullable=True),
    sa.ForeignKeyConstraint(['source_recommendation_id'], ['recommendations.id'], ondelete='CASCADE'),
    sa.ForeignKeyConstraint(['user_id'], ['users.id'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id')
    )
