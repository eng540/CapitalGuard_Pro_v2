...
revision = '20251004_implement_v3_multi_tenant_schema'
...
def upgrade() -> None:
    ...
    # ✅ IDEMPOTENT FIX: Create new ENUM types only if they don't already exist.
    op.execute("""
        DO $$
        BEGIN
    ...
    op.add_column('channels', sa.Column('analyst_id', sa.Integer(), nullable=False, server_default='1')) # Add a temporary default
    op.alter_column('channels', 'analyst_id', server_default=None) # Remove the default
    op.drop_constraint('channels_user_id_fkey', 'channels', type_='foreignkey')
    op.create_foreign_key('fk_channels_analyst_id_users', 'channels', 'users', ['analyst_id'], ['id'])
    op.drop_column('channels', 'user_id')

    op.add_column('recommendations', sa.Column('analyst_id', sa.Integer(), nullable=False, server_default='1')) # Add a temporary default
    op.alter_column('recommendations', 'analyst_id', server_default=None) # Remove the default
    ...
    op.create_foreign_key('fk_recommendations_analyst_id_users', 'recommendations', 'users', ['analyst_id'], ['id'])
...
def downgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
    op.add_column('users', sa.Column('email', sa.VARCHAR(), autoincrement=False, nullable=False, server_default=sa.text("''::character varying")))
    ...
    op.drop_constraint('fk_recommendations_analyst_id_users', 'recommendations', type_='foreignkey')
    ...
    op.drop_constraint('fk_channels_analyst_id_users', 'channels', type_='foreignkey')
    ...
    # ✅ IDEMPOTENT FIX: Drop new ENUM types only if they exist.
