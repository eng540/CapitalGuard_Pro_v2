...
# Version: v6.0.0-EVENT-TRIGGER
# ✅ THE FIX: Added global triggers to instantly wake up the Price Streamer upon trade creation.
...
# 🔥 استيراد المحفز العالمي لمحرك الأسعار
from capitalguard.infrastructure.sched.price_streamer import trigger_price_update
...
class CreationService:
    ...
    async def background_publish_and_index(self, rec_id: int, user_db_id: int, target_channel_ids: Optional[Set[int]] = None):
        ...
        try:
            with session_scope() as session:
                ...
                if rec_entity:
                    try:
                        _, report = await self._publish_recommendation(session, rec_entity, user_db_id, target_channel_ids)
                ...
                try:
                    ...
                    # 🔥 الإيقاظ اللحظي للمراقب بمجرد أن تصبح التوصية حقيقية
                ...
                try:
                    ...
                    if publish_error: msg += f"\n⚠️ تنبيه: فشل النشر في القنوات ({publish_error})."
                    elif success_count == 0: msg += "\nℹ️ لم يتم النشر في أي قناة، لكن الصفقة تعمل."
    ...
    async def create_trade_from_forwarding_async(
        self, user_id: str, trade_data: Dict[str, Any], original_text: Optional[str], 
        db_session: Session, status_to_set: str, original_published_at: Optional[datetime],
        ...
        try:
            ...
            if channel_info and channel_info.get('id'):
                ...
                if not watched_channel:
                    watched_channel = WatchedChannel(user_id=trader_user.id, telegram_channel_id=channel_tg_id, channel_title=channel_info.get('title'), is_active=True)
                    db_session.add(watched_channel)
                    db_session.flush() 

            new_trade = UserTrade(
                user_id=trader_user.id, asset=trade_data['asset'], side=trade_data['side'],
                entry=entry_dec, stop_loss=sl_dec, targets=targets_for_db, status=UserTradeStatusEnum[status_to_set],
    ...
    async def create_trade_from_recommendation(self, user_id: str, rec_id: int, db_session: Session) -> Dict[str, Any]:
        ...
        try:
            ...
            if rec_status == RecommendationStatusEnum.PENDING: user_trade_status, user_trade_activated_at = UserTradeStatusEnum.PENDING_ACTIVATION, None
            elif rec_status == RecommendationStatusEnum.ACTIVE: user_trade_status, user_trade_activated_at = UserTradeStatusEnum.ACTIVATED, rec_orm.activated_at or datetime.now(timezone.utc)
            else: return {'success': False, 'error': 'This signal is already closed.'}

            new_trade = UserTrade( 
                user_id=trader_user.id, asset=rec_orm.asset, side=rec_orm.side, entry=rec_orm.entry, 
                stop_loss=rec_orm.stop_loss, targets=rec_orm.targets, status=user_trade_status, 
                activated_at=user_trade_activated_at, original_published_at=rec_orm.created_at, source_recommendation_id=rec_orm.id 
            )
            db_session.add(new_trade)
            db_session.flush()
