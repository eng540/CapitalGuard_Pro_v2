...
class TradeService:
    ...
    def create_and_publish_recommendation(
        ...
        log.info("Creating recommendation: asset=%s side=%s order_type=%s user=%s", asset, side, order_type, user_id)
    ...
    def move_sl_to_be(self, rec_id: int) -> Optional[Recommendation]:
        ...
        note = f"\n- SL moved to BE on {datetime.utcnow().strftime('%Y-%m-%d %H:%M')} UTC."
        rec.notes = (rec.notes or "") + note
        updated_rec = self.repo.update(rec)
        self._update_cards(updated_rec)
        log.info("Rec #%s SL moved to BE (entry=%s)", rec_id, rec.entry.value)
        return updated_rec
    ...
    def add_partial_close_note(self, rec_id: int) -> Optional[Recommendation]:
        ...
        if not rec or rec.status == RecommendationStatus.CLOSED:
            return None
        note = f"\n- 50% of position closed on {datetime.utcnow().strftime('%Y-%m-%d %H:%M')} UTC."
        rec.notes = (rec.notes or "") + note
        updated_rec = self.repo.update(rec)
        self._update_cards(updated_rec)
