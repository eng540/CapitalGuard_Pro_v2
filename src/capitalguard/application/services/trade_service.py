...
# src/capitalguard/application/services/trade_service.py v31.0.0 - Parsing/UserTrade Integration
"""
TradeService v31.0.0 - Integrates ParsingService flow and UserTrade management.
✅ NEW: Added `create_trade_from_forwarding_async` to save parsed trades.
✅ NEW: Added `close_user_trade_async` to handle closing personal trades.
✅ Includes previous fixes for Decimal handling, deep-linking, validation, and helpers.
"""
...
from capitalguard.domain.entities import (
    ...
    OrderType, ExitStrategy, UserType as UserTypeEntity # Ensure UserTypeEntity is imported
) #
...
def _to_decimal(value: Any, default: Decimal = Decimal('0')) -> Decimal:
    ...
    try:
        ...
        # Keep logging minimal for helpers unless critical
...
def _pct(entry: Any, target_price: Any, side: str) -> float:
    ...
    try:
        ...
        if not entry_dec.is_finite() or entry_dec.is_zero() or not target_dec.is_finite():
            return 0.0
        # Ensure side is a string before calling upper()
...
class TradeService:
    ...
    def __init__(
        self,
        repo: RecommendationRepository, # Expecting instance now based on previous discussion
        notifier: Any,
    ...
    async def _commit_and_dispatch(self, db_session: Session, rec_orm: Union[Recommendation, UserTrade], rebuild_alerts: bool = True):
        """Commits changes, refreshes ORM, updates alerts, notifies UI (if Recommendation)."""
        item_id = getattr(rec_orm, 'id', 'N/A')
        item_type = type(rec_orm).__name__
        try:
            db_session.commit()
        ...
        if isinstance(rec_orm, Recommendation):
    ...
    async def notify_card_update(self, rec_entity: RecommendationEntity, db_session: Session):
        ...
        # (Implementation remains same as v30.9)
        ...
        try:
            ...
            for res in results:
                if isinstance(res, Exception): logger.error(f"notify_card_update task failed for Rec ID {rec_entity.id}: {res}", exc_info=False)
        except Exception as e: logger.error(f"Error fetching/updating published messages for Rec ID {rec_entity.id}: {e}", exc_info=True)
    ...
    def _validate_recommendation_data(self, side: str, entry: Decimal, stop_loss: Decimal, targets: List[Dict[str, Any]]):
        ...
        # (Implementation remains same as v30.9 - ensures prices are positive, finite Decimals,
        # targets are list of dicts with valid prices/percents, SL/TPs are logical vs Entry/Side, R:R > 0.1)
        ...
        for i, t in enumerate(targets):
            ...
            except (ValueError, TypeError): raise ValueError(f"Target {i+1} close % invalid.")
        ...
        risk = abs(entry - stop_loss); first_tp = min(target_prices) if side_upper == "LONG" else max(target_prices); reward = abs(first_tp - entry)
    ...
    async def _publish_recommendation(self, session: Session, rec_entity: RecommendationEntity, user_db_id: int, target_channel_ids: Optional[Set[int]] = None) -> Tuple[RecommendationEntity, Dict]:
        """Publishes a recommendation entity to analyst channels."""
        # (Implementation remains same as v30.9)
        ...
        tasks = []; channel_map = {ch.telegram_channel_id: ch for ch in channels_to_publish}
        ...
        for i, channel_id in enumerate(channel_map.keys()):
            result = results[i]
            if isinstance(result, Exception): logger.exception(f"Failed to publish Rec {rec_entity.id} to channel {channel_id}: {result}"); report["failed"].append({"channel_id": channel_id, "reason": str(result)})
            elif isinstance(result, tuple) and len(result) == 2: session.add(PublishedMessage(recommendation_id=rec_entity.id, telegram_channel_id=result[0], telegram_message_id=result[1])); report["success"].append({"channel_id": channel_id, "message_id": result[1]})
            else: reason = f"Notifier unexpected result: {type(result)}"; logger.error(f"Failed to publish Rec {rec_entity.id} to channel {channel_id}: {reason}"); report["failed"].append({"channel_id": channel_id, "reason": reason})
    ...
    async def create_and_publish_recommendation_async(self, user_id: str, db_session: Session, **kwargs) -> Tuple[Optional[RecommendationEntity], Dict]:
        ...
        # (Implementation remains same as v30.9 - validation, MARKET price fetch, ORM creation, logging, publishing)
        user = UserRepository(db_session).find_by_telegram_id(_parse_int_user_id(user_id))
        if not user or user.user_type != UserTypeEntity.ANALYST: raise ValueError("Only analysts can create recommendations.")

        entry_price_in = _to_decimal(kwargs['entry']); sl_price = _to_decimal(kwargs['stop_loss'])
        targets_list_in = kwargs['targets']; targets_list_validated = [{'price': _to_decimal(t['price']), 'close_percent': t.get('close_percent', 0.0)} for t in targets_list_in]
        asset = kwargs['asset'].strip().upper(); side = kwargs['side'].upper(); market = kwargs.get('market', 'Futures'); order_type_enum = OrderTypeEnum[kwargs['order_type'].upper()]
        ...
        if exit_strategy_val is None: exit_strategy_enum = ExitStrategyEnum.CLOSE_AT_FINAL_TP
            ...
            except KeyError: raise ValueError(f"Unsupported exit_strategy: {exit_strategy_val}")
        ...
        db_session.add(RecommendationEvent( recommendation_id=rec_orm.id, event_type="CREATED_ACTIVE" if status == RecommendationStatusEnum.ACTIVE else "CREATED_PENDING", event_data={'entry': str(final_entry)} )); db_session.flush(); db_session.refresh(rec_orm)

        created_rec_entity = self.repo._to_entity(rec_orm)
        if not created_rec_entity: raise RuntimeError(f"Failed to convert new ORM Rec ID {rec_orm.id} to entity.")
    ...
    async def create_trade_from_forwarding_async(
        ...
        """Creates a UserTrade from successfully parsed forwarded data."""
        ...
        try:
            ...
            # No need to build alerts index for UserTrade
            ...
            db_session.rollback() # Rollback on general error
    ...
    async def create_trade_from_recommendation(self, user_id: str, rec_id: int, db_session: Session) -> Dict[str, Any]:
        ...
        try:
            ...
            return {'success': True, 'trade_id': new_trade.id, 'asset': new_trade.asset}
        except Exception as e: logger.error(f"Error creating trade from rec for user {user_id}, rec {rec_id}: {e}", exc_info=True); db_session.rollback(); return {'success': False, 'error': 'Internal error tracking signal.'}
    ...
    async def close_user_trade_async(
        ...
        trade.close_price = exit_price # Store Decimal directly
        trade.closed_at = datetime.now(timezone.utc)

        # Calculate and store final PnL
        try:
            # Use Decimal for calculation
            entry_for_calc = _to_decimal(trade.entry) # Ensure entry is Decimal
            pnl_float = _pct(entry_for_calc, exit_price, trade.side)
            # Store PnL with appropriate precision
            trade.pnl_percentage = Decimal(f"{pnl_float:.4f}") # Use 4 decimal places for PnL %
        ...
        # Commit happens via _commit_and_dispatch or session_scope
        # No need to rebuild alerts index for UserTrade
        db_session.flush() # Ensure changes are flushed before potentially returning
        # Don't call _commit_and_dispatch here, let UOW handle commit
        return trade


    # --- Update Operations (Analyst - Keep implementations from v30.9) ---
    async def update_sl_for_user_async(self, rec_id: int, user_id: str, new_sl: Decimal, db_session: Optional[Session] = None) -> RecommendationEntity:
        ...
        user = UserRepository(db_session).find_by_telegram_id(_parse_int_user_id(user_id)); # ... (rest of validation) ...
        if not user: raise ValueError("User not found.")
        ...
        try: targets_list = [{'price': _to_decimal(t.get('price')), 'close_percent': t.get('close_percent', 0.0)} for t in (rec_orm.targets or [])]; self._validate_recommendation_data(rec_orm.side, rec_orm.entry, new_sl, targets_list)
    ...
    async def update_targets_for_user_async(self, rec_id: int, user_id: str, new_targets: List[Dict[str, Any]], db_session: Session) -> RecommendationEntity:
        ...
        if rec_orm.analyst_id != user.id: raise ValueError("Access denied.")
        if rec_orm.status != RecommendationStatusEnum.ACTIVE: raise ValueError("Only ACTIVE.")
        try: targets_validated = [{'price': _to_decimal(t['price']), 'close_percent': t.get('close_percent', 0.0)} for t in new_targets]; self._validate_recommendation_data(rec_orm.side, rec_orm.entry, rec_orm.stop_loss, targets_validated)
    ...
    async def update_entry_and_notes_async(self, rec_id: int, user_id: str, new_entry: Optional[Decimal], new_notes: Optional[str], db_session: Session) -> RecommendationEntity:
        ...
        if new_entry is not None:
            if rec_orm.status != RecommendationStatusEnum.PENDING: raise ValueError("Entry only editable for PENDING.")
            try: targets_list = [{'price': _to_decimal(t.get('price')), 'close_percent': t.get('close_percent', 0.0)} for t in (rec_orm.targets or [])]; self._validate_recommendation_data(rec_orm.side, new_entry, rec_orm.stop_loss, targets_list)
        ...
        if updated: db_session.add(RecommendationEvent(recommendation_id=rec_id, event_type="DATA_UPDATED", event_data=event_data)); self.notify_reply(rec_id, f"✏️ Data for #{rec_orm.asset} updated.", db_session); await self._commit_and_dispatch(db_session, rec_orm, rebuild_alerts=(new_entry is not None));
        else: logger.debug(f"No changes for update_entry_and_notes Rec ID {rec_id}.")
    ...
    async def set_exit_strategy_async(self, rec_id: int, user_id: str, mode: str, price: Optional[Decimal] = None, trailing_value: Optional[Decimal] = None, active: bool = True, session: Optional[Session] = None) -> RecommendationEntity:
        ...
        user = UserRepository(session).find_by_telegram_id(_parse_int_user_id(user_id)); # ... (rest of validation) ...
        if not user: raise ValueError("User not found.")
        ...
        rec.profit_stop_mode = mode_upper if active else "NONE"; rec.profit_stop_price = price if active and mode_upper == "FIXED" else None; rec.profit_stop_trailing_value = trailing_value if active and mode_upper == "TRAILING" else None; rec.profit_stop_active = active;
        ...
        if active: msg = f"📈 Exit strategy for #{rec.asset} set: {mode_upper}"; # ... (add price/value to msg) ...
        else: msg = f"❌ Exit strategy for #{rec.asset} cancelled."
    ...
    async def move_sl_to_breakeven_async(self, rec_id: int, db_session: Optional[Session] = None) -> RecommendationEntity:
        # (Implementation remains same as v30.9 - uses buffer)
        if db_session is None:
        ...
        if not entry_dec.is_finite() or entry_dec <= 0 or not current_sl_dec.is_finite(): raise ValueError("Invalid entry/SL for BE calc.")
        ...
        if is_improvement:
            ...
            if not analyst_uid: raise RuntimeError(f"Cannot perform BE for Rec ID {rec_id}: Analyst info missing.")
        ...
        else: logger.info(f"SL for Rec #{rec_id} already at/better than BE target {new_sl_target:g}. No action."); return self.repo._to_entity(rec_orm)


    # --- Closing Operations (Keep implementations from v30.9) ---
    ...
    async def partial_close_async(self, rec_id: int, user_id: str, close_percent: Decimal, price: Decimal, db_session: Session, triggered_by: str = "MANUAL") -> RecommendationEntity:
        ...
        if rec_orm.analyst_id != user.id: raise ValueError("Access denied.")
        if rec_orm.status != RecommendationStatusEnum.ACTIVE: raise ValueError("Only ACTIVE.")
        current_open_percent = _to_decimal(rec_orm.open_size_percent); close_percent_dec = _to_decimal(close_percent); price_dec = _to_decimal(price);
        ...
        notif_icon = "💰 Profit" if pnl_on_part >= 0 else "⚠️ Loss Mgt"; notif_text = f"{notif_icon} Partial Close on #{rec_orm.asset}. Closed {actual_close_percent:g}% at {_format_price(price_dec)} ({pnl_formatted}).\nRemaining: {rec_orm.open_size_percent:g}%"; self.notify_reply(rec_id, notif_text, db_session);
    ...
    async def process_tp_hit_event(self, item_id: int, target_index: int, price: Decimal):
        ...
        with session_scope() as s:
            ...
            if any(e.event_type == event_type for e in (rec_orm.events or [])): logger.debug(f"TP event {event_type} already processed for {item_id}"); return
            s.add(RecommendationEvent(recommendation_id=rec_orm.id, event_type=event_type, event_data={"price": float(price)})); self.notify_reply(rec_orm.id, f"🎯 Signal #{rec_orm.asset} hit TP{target_index} at {_format_price(price)}!", db_session=s);
            ...
            if not analyst_uid_str: logger.error(f"Cannot auto close/partial for TP on Rec {item_id}: Analyst missing."); await self._commit_and_dispatch(s, rec_orm, False); return # Commit event only
            ...
            elif close_percent <= 0: await self._commit_and_dispatch(s, rec_orm, False); # Commit event if no partial/full close


    # --- Read Utilities (Keep implementations from v30.9) ---
    def get_open_positions_for_user(self, db_session: Session, user_telegram_id: str) -> List[RecommendationEntity]:
        # (Implementation remains same as v30.9 - fetches Recs + Trades, converts Trade to Entity)
        ...
        for trade in trades_orm:
            ...
            except Exception as conv_err: logger.error(f"Failed conv UserTrade {trade.id}: {conv_err}", exc_info=True);
    ...
    def get_position_details_for_user(self, db_session: Session, user_telegram_id: str, position_type: str, position_id: int) -> Optional[RecommendationEntity]:
        # (Implementation remains same as v30.9 - fetches details, converts Trade to Entity, checks ownership)
        ...
        if position_type == 'rec':
            ...
            if not rec_orm or rec_orm.analyst_id != user.id: return None # Strict owner check for rec
