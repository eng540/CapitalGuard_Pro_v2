...
def _ensure_token_maps(context: ContextTypes.DEFAULT_TYPE) -> None:
    if REV_TOKENS_MAP not in context.bot_data:
        context.bot_data[REV_TOKENS_MAP] = {}
    if REV_TOKENS_REVERSE not in context.bot_data:
        context.bot_data[REV_TOKENS_REVERSE] = {}
...
def _resolve_review_key_from_token(context: ContextTypes.DEFAULT_TYPE, token: str) -> Optional[str]:
    _ensure_token_maps(context)
...
def _draft_for_review_text(draft: Dict[str, Any]) -> Dict[str, Any]:
    """
    Convert draft['targets'] (list of dicts) into objects with attribute access
    so build_review_text_with_price/build_review_text can format them safely.
    Does not mutate the original draft.
    """
    data = dict(draft)
    tps = data.get("targets") or []
    if tps and isinstance(tps[0], dict):
        data["targets"] = [
            types.SimpleNamespace(
                price=float(tp.get("price")),
                close_percent=float(tp.get("close_percent", 0.0))
            )
            for tp in tps
        ]
    return data
...
async def start_quick_entrypoint(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    _clean_conversation_state(context)
    context.user_data[CONVERSATION_DATA_KEY] = {}
    context.user_data['input_mode'] = 'quick'
    await update.message.reply_text(
        "⚡️ وضع الأمر السريع.\n\nأرسل الآن توصيتك الكاملة في رسالة واحدة (تبدأ بـ /rec)."
    )
...
async def received_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if mode == 'quick':
        ...
        if not data:
            await update.message.reply_text("❌ صيغة غير صحيحة. يجب أن تبدأ الرسالة بـ `/rec` وتحتوي على جميع الحقول.")
            return AWAIT_TEXT_INPUT
    elif mode == 'editor':
        data = parse_text_editor(text)
        if not data:
            await update.message.reply_text("❌ تعذّر تحليل النص. تأكد من وجود الحقول الأساسية.")
            return AWAIT_TEXT_INPUT
    else:
...
async def show_review_card(update: Update, context: ContextTypes.DEFAULT_TYPE, is_edit: bool = False) -> int:
    ...
    # Prepare a safe copy for UI builder (targets as objects with .price)
    data_for_view = _draft_for_review_text(data)

    price_service = get_service(context, "price_service")
    preview_price = price_service.get_preview_price(data_for_view["asset"], data_for_view.get("market", "Futures"))
    review_text = build_review_text_with_price(data_for_view, preview_price)

    if not review_key:
        review_key = str(uuid.uuid4())
        context.user_data['current_review_key'] = review_key
        context.bot_data[review_key] = dict(data)  # original draft preserved
...
async def asset_chosen_button(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    draft = context.user_data.get(CONVERSATION_DATA_KEY, {}) or {}
...
async def asset_chosen_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not market_data_service.is_valid_symbol(raw_asset, preferred_market):
        await update.message.reply_text(
            f"❌ الرمز '{raw_asset}' غير صالح أو غير متوفر في السوق ({preferred_market})."
        )
        return I_ASSET
...
async def add_notes_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not review_key or review_key not in context.bot_data:
        await query.message.edit_text("❌ انتهت صلاحية البطاقة.")
        return ConversationHandler.END
    context.user_data['current_review_key'] = review_key
...
async def publish_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        targets_data = draft["targets"]  # already list[dict] from parser
        ...
        _, report = trade_service.publish_recommendation(
            rec_id=saved_rec.id, user_id=str(update.effective_user.id)
        )

        if report.get("success"):
            success_count = len(report["success"])
            await query.edit_message_text(
                f"✅ تم الحفظ بنجاح ونشر التوصية #{saved_rec.id} إلى {success_count} قناة."
            )
        else:
...
async def channel_picker_confirm_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    selected: Set[int] = context.user_data.get(sel_key, set())
    if not draft:
        await query.edit_message_text("❌ انتهت صلاحية البطاقة.")
        return ConversationHandler.END
    if not selected:
        await query.edit_message_text("⚠️ لم تختر أي قناة.")
        return
    trade_service = get_service(context, "trade_service")
    try:
        live_price = get_service(context, "price_service").get_cached_price(
        ...
        trade_service.publish_recommendation(
            rec_id=rec.id, user_id=str(query.from_user.id), channel_ids=list(selected)
        )
...
def register_conversation_handlers(app: Application):
    creation_conv_handler = ConversationHandler(
        ...
        states={
            ...
            AWAIT_TEXT_INPUT: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, received_text_input)
            ],
            I_ASSET: [
