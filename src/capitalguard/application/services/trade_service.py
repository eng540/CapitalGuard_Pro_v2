    ...
    def create(self, asset: str, side: str, entry: float, stop_loss: float,  
        ...
        if self.notifier:  
            try:  
                ...
                # ✅ MODIFIED: Log the error instead of passing silently  
                logging.error(f"Failed to send creation notification for recommendation #{saved.id}: {e}")  
        return saved  
    ...
    def close(self, rec_id: int, exit_price: float) -> Recommendation:  
        ...
        if self.notifier:  
            try:  
                ...
                # ✅ MODIFIED: Log the error instead of passing silently  
                logging.error(f"Failed to send close notification for recommendation #{saved.id}: {e}")  
        return saved  
