...
class TradeService:
    ...
    def create_and_publish_recommendation(self, asset: str, side: str, market: str, entry: float, stop_loss: float, targets: List[float], notes: Optional[str], user_id: Optional[str]) -> Recommendation:
        ...
        rec_to_save = Recommendation(asset=Symbol(asset), side=Side(side), entry=Price(entry), stop_loss=Price(stop_loss), targets=Targets(targets), market=market, notes=notes, user_id=user_id, status=RecommendationStatus.PENDING)
    ...
    def move_sl_to_be(self, rec_id: int) -> Optional[Recommendation]:
        rec = self.repo.get(rec_id)
        # ✅ --- FIX: Compare Enum object directly ---
    ...
    def add_partial_close_note(self, rec_id: int) -> Optional[Recommendation]:
        ...
        if not rec or rec.status == RecommendationStatus.CLOSED:
            return None
        note = f"\n- 50% of position closed on {datetime.now().strftime('%Y-%m-%d %H:%M')} to secure profits."
        rec.notes = (rec.notes or "") + note
        updated_rec = self.repo.update(rec)
        self._update_cards(updated_rec)
        return updated_rec
