# src/capitalguard/application/services/trade_service.py (v25.0 - FINAL COMPLETE)
"""
TradeService - الإصدار النهائي الكامل مع دعم توصيات الظل
"""
...
class TradeService:
    ...
    async def track_forwarded_trade(self, user_id: str, trade_data: Dict[str, Any], db_session: Session) -> Dict[str, Any]:
        ...
        try:
            ...
            # 2. إنشاء سجل "توصية الظل" مع الحقل الجديد is_shadow
            shadow_rec = Recommendation(
                ...
                market="Futures",
                is_shadow=True, # ✅ الآن الحقل موجود في قاعدة البيانات
    ...
    def get_all_shadow_recommendations_for_user(self, db_session: Session, user_telegram_id: str) -> List[Dict[str, Any]]:
        """الحصول على جميع توصيات الظل المرتبطة بصفقات المستخدم"""
        user = self._get_user_by_telegram_id(db_session, user_telegram_id)
        if not user:
            return []
            
        # الحصول على جميع صفقات المستخدم التي لها توصيات ظل مرتبطة
        trades_with_shadows = db_session.query(UserTrade).filter(
            UserTrade.user_id == user.id,
            UserTrade.source_recommendation_id.isnot(None)
        ).all()
        
        result = []
        for trade in trades_with_shadows:
            shadow_rec = self.repo.get(db_session, trade.source_recommendation_id)
            if shadow_rec and getattr(shadow_rec, 'is_shadow', False):
                result.append({
                    'trade_id': trade.id,
                    'shadow_recommendation_id': shadow_rec.id,
                    'asset': shadow_rec.asset,
                    'side': shadow_rec.side,
                    'entry': float(shadow_rec.entry),
                    'stop_loss': float(shadow_rec.stop_loss),
                    'status': shadow_rec.status.value,
