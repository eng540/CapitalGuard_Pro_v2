...
class TradeService:
    ...
    def _update_cards(self, rec: Recommendation):
        ...
        if rec.user_id and rec.user_id.isdigit():
            ...
            self.notifier.send_private_message(
                chat_id=int(rec.user_id), rec=rec, keyboard=analyst_keyboard,
    ...
    def create_and_publish_recommendation(
        ...
        # ✅ --- FIX: Explicitly define status and entry price based on order type ---
    ...
    # --- (All other methods: close, list_open, etc., are unchanged from the last correct version) ---
