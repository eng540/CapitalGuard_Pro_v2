# --- START OF FINAL, UPDATED FILE (V23): src/capitalguard/application/services/alert_service.py ---
import logging
import os
import asyncio
    ...
    def check_once(self) -> int:
        ...
        for rec in active_recs:
            ...
            try:
                ...
                # --- Near-touch alerts for analyst (one-time) ---
                if near_alert_pct > 0:
                    ...
                    if not self.repo.check_if_event_exists(rec.id, near_sl_event):
                        ...
                        if is_near_sl:
                            log.info(f"Near SL for rec #{rec.id}. Logging event and notifying analyst.", rec.id)
                    ...
                    if rec.targets.values and not self.repo.check_if_event_exists(rec.id, near_tp1_event):
                        ...
                        if is_near_tp1:
                            log.info(f"Near TP1 for rec #{rec.id}. Logging event and notifying analyst.", rec.id)
...
# --- END OF FINAL, UPDATED FILE (V23): src/capitalguard/application/services/alert_service.py ---