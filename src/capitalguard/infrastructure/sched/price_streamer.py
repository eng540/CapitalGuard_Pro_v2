...
class PriceStreamer:
    ...
    async def _listen_symbol_events(self) -> None:
        """
        [STEP-3A] يستمع لـ Redis channel "cg:symbol_update".

        الإصلاح:
            قبل: client = aioredis.from_url() بدون إغلاق صريح عند الخروج.
                 كل انقطاع = connection pool جديدة تُضاف ولا تُغلق القديمة.
                 100 انقطاع = 100 connection pools = Redis يرفض connections جديدة.

            بعد: client يُنشأ في try، ويُغلق صراحةً في finally في كل الأحوال:
                 - انتهاء طبيعي
                 - استثناء
                 - CancelledError
                 - timeout
        ...
        المعيار المالي:
            كل resource خارجي (DB، Redis، WS) يجب أن يُغلق صراحةً.
            Connection leaks في أنظمة realtime تُسبب رفض الاتصالات
            وتوقف استقبال أحداث السوق.
        """
        log.info("PriceStreamer: [STEP-3A] Pub/Sub listener starting (leak-free).")

        while self._running:
            client = None  # [STEP-3A] تعريف خارج try لضمان وصول finally إليه
            try:
                ...
                # [STEP-3A] إنشاء client مع timeout صريح للاتصال
                ...
                # [STEP-3A] إغلاق صريح في كل الأحوال — يمنع connection leak
                if client is not None:
                    try:
                        await client.aclose()
                        log.debug("PriceStreamer: Redis client closed cleanly.")
                    except Exception as close_err:
                        log.debug(
                            "PriceStreamer: Redis client close error (ignored): %s",
                            close_err,
                        )
    ...
    async def _handle_price(
        ...
        await self.price_queue.put({
            ...
            "high":   high,
            "close":  close,
            "ts":     int(asyncio.get_running_loop().time()),  # [STEP-6B] كان get_event_loop() — deprecated في Python 3.10+
        })
