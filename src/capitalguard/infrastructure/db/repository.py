...
class RecommendationRepository:
    ...
    def _to_entity(self, row: RecommendationORM) -> Recommendation:
        """Convert a SQLAlchemy ORM row into a domain Recommendation entity."""
    ...
    @staticmethod
    def _apply_entity_to_row(row: RecommendationORM, rec: Recommendation) -> None:
        """Apply domain entity fields onto an ORM row (used by add/update)."""
        ...
        # Assign Enum objects directly (ORM columns are Enum)
    ...
    def add(self, rec: Recommendation) -> Recommendation:
        ...
        with SessionLocal() as s:
            try:
                ...
                self._apply_entity_to_row(row, rec)
                s.add(row)
                s.commit()
                s.refresh(row)
    ...
    def list_open(self) -> List[Recommendation]:
        """List recommendations that are either PENDING or ACTIVE."""
        with SessionLocal() as s:
            rows = (
    ...
    def list_all(self, symbol: Optional[str] = None, status: Optional[str] = None) -> List[Recommendation]:
        ...
        with SessionLocal() as s:
            ...
            if status:
                # Accept either Enum or string like "PENDING"
                status_enum: Optional[RecommendationStatus] = None
                if isinstance(status, RecommendationStatus):
                    status_enum = status
    ...
    def get_recent_assets_for_user(self, user_id: str, limit: int = 5) -> List[str]:
        """Fetch the most recently used unique assets for a given user."""
        with SessionLocal() as s:
            subquery = (
                s.query(
