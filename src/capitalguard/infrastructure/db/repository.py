...
class UserRepository:
    ...
    def find_or_create(self, telegram_id: int, **kwargs) -> User:
        ...
        if user:
            changed = False
            if not getattr(user, "email", None): user.email = placeholder_email; changed = True
            if kwargs.get("user_type") and getattr(user, "user_type", None) != kwargs["user_type"]: user.user_type = kwargs["user_type"]; changed = True
            if getattr(user, "is_active", True) is False: user.is_active = True; changed = True
            if kwargs.get("first_name") is not None and getattr(user, "first_name", None) != kwargs["first_name"]: user.first_name = kwargs["first_name"]; changed = True
            if changed: self.session.commit(); self.session.refresh(user)
            return user
...
class ChannelRepository:
    ...
    def find_by_username(self, username: str) -> Optional[Channel]:
        ...
        return self.session.query(Channel).filter(sa.func.lower(Channel.username) == clean).first()
    ...
    def add(self, rec: Recommendation) -> Recommendation:
        ...
        with SessionLocal() as s:
            ...
            row = RecommendationORM(
                ...
                message_id=rec.message_id, published_at=rec.published_at, market=rec.market,
