# --- START OF FINAL, COMPLETE, AND CONCURRENCY-SAFE FILE (Version 13.3.1) ---
...
class UserRepository:
    ...
    def find_or_create(self, telegram_id: int, **kwargs) -> User:
        ...
        try:
            ...
            new_user = User(telegram_id=telegram_id, email=kwargs.get("email") or f"tg{telegram_id}@telegram.local", is_active=False, first_name=kwargs.get("first_name"))
...
class ChannelRepository:
    ...
    def add(self, owner_user_id: int, telegram_channel_id: int, **kwargs) -> Channel:
        existing_channel = self.session.query(Channel).filter(Channel.telegram_channel_id == telegram_channel_id).first()
        if existing_channel:
            ...
            new_channel = Channel(user_id=owner_user_id, telegram_channel_id=telegram_channel_id, title=kwargs.get("title"), username=kwargs.get("username"), is_active=True, last_verified_at=datetime.now(timezone.utc))
            self.session.add(new_channel)
            self.session.flush()
            self.session.refresh(new_channel)
            return new_channel
    def set_active(self, owner_user_id: int, telegram_channel_id: int, is_active: bool):
        channel = self.session.query(Channel).filter(Channel.user_id == owner_user_id, Channel.telegram_channel_id == telegram_channel_id).first()
        if channel:
            channel.is_active = is_active
            self.session.flush()

class RecommendationRepository:
    # ... (بقية الكلاس تبقى كما هي)
    @staticmethod
    def _to_entity(row: RecommendationORM) -> Optional[Recommendation]:
        ...
        return Recommendation(id=row.id, asset=Symbol(row.asset), side=Side(row.side), entry=Price(row.entry), stop_loss=Price(row.stop_loss), targets=targets_vo, order_type=OrderType(row.order_type), status=RecommendationStatus(row.status), market=row.market, notes=row.notes, user_id=user_telegram_id, created_at=row.created_at, updated_at=row.updated_at, exit_price=row.exit_price, activated_at=row.activated_at, closed_at=row.closed_at, alert_meta=dict(row.alert_meta or {}), highest_price_reached=row.highest_price_reached, lowest_price_reached=row.lowest_price_reached, exit_strategy=ExitStrategy(row.exit_strategy), profit_stop_price=row.profit_stop_price, open_size_percent=row.open_size_percent, events=getattr(row, 'events', None))
    ...
    def add_with_event(self, session: Session, rec: Recommendation) -> Recommendation:
        ...
        row = RecommendationORM(user_id=user.id, asset=rec.asset.value, side=rec.side.value, entry=rec.entry.value, stop_loss=rec.stop_loss.value, targets=[v.__dict__ for v in rec.targets.values], order_type=rec.order_type.value, status=rec.status.value, market=rec.market, notes=rec.notes, exit_strategy=rec.exit_strategy.value, open_size_percent=rec.open_size_percent, activated_at=rec.activated_at, highest_price_reached=rec.highest_price_reached, lowest_price_reached=rec.lowest_price_reached)
        session.add(row)
        session.flush()
    ...
    def get(self, session: Session, rec_id: int) -> Optional[Recommendation]:
        row = session.query(RecommendationORM).options(joinedload(RecommendationORM.user), selectinload(RecommendationORM.events)).filter(RecommendationORM.id == rec_id).first()
        return self._to_entity(row)
    ...
    def get_by_id_for_user(self, session: Session, rec_id: int, user_telegram_id: Union[int, str]) -> Optional[Recommendation]:
        ...
        row = session.query(RecommendationORM).options(joinedload(RecommendationORM.user), selectinload(RecommendationORM.events)).filter(RecommendationORM.id == rec_id, RecommendationORM.user_id == user.id).first()
        return self._to_entity(row)
        
    # ✅ CRITICAL FIX: Removed the unnecessary 'options' parameter.
