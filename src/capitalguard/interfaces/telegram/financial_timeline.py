...
def _time(value: Any) -> str:
    ...
    except (TypeError, ValueError, OverflowError): return _text(value)
...
def _event_line(event: Any) -> str:
    ...
    if pnl is not None:
        try: line += f" ({float(pnl):+.2f}%)"
        except (TypeError, ValueError): line += f" ({_text(pnl)}%)"
    return line
...
def format_financial_replay_result(replay: Mapping[str, Any] | None, *, events: Any = None) -> list[str]:
    ...
    if net_pnl is not None:
        try:
            value = float(net_pnl); marker = "💚 ربح" if value > 0 else "💔 خسارة" if value < 0 else "➖ تعادل"
            lines.append(f"النتيجة الصافية: <code>{value:+.2f}%</code> ({marker})")
        except (TypeError, ValueError): lines.append(f"النتيجة الصافية: <code>{_text(net_pnl)}</code>")
    elif lifecycle == "PENDING_ORDER": lines.append("النتيجة الصافية: <code>0.00%</code> — لم يدخل رأس المال حيز المخاطرة.")
    ...
    elif timeline: lines += ["", "<b>📜 سجل مسار الصفقة والأحداث</b>", *timeline]
    if lifecycle in _LIFECYCLE_LABELS: lines.append(f"دورة الحياة: <code>{lifecycle}</code>")
    if replay.get("coverage_status"): lines.append(f"التغطية: <code>{_text(replay['coverage_status'])}</code>")
    return lines
