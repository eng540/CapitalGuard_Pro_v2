# src/capitalguard/interfaces/telegram/forwarding_handlers.py (v25.9 - FINAL & CORRECTED)
"""
Handles the conversation flow for parsing a forwarded message and creating a
personal user trade for tracking, with corrected UoW decorator usage.
"""
...
async def forwarding_entry_point(update: Update, context: ContextTypes.DEFAULT_TYPE, db_session, **kwargs) -> int:
    ...
    await processing_msg.edit_text(confirmation_text, reply_markup=keyboard, parse_mode='HTML')
    return AWAITING_CONFIRMATION

@uow_transaction
async def handle_confirmation(update: Update, context: ContextTypes.DEFAULT_TYPE, **kwargs) -> int:
    ...
    # The @uow_transaction on the service method will provide it.
    result = await trade_service.create_trade_from_forwarding(user_id, trade_data)
...
def create_forwarding_conversation_handler() -> ConversationHandler:
