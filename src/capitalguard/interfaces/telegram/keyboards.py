# src/capitalguard/interfaces/telegram/keyboards.py (v21.7.1 - Final Production)
"""
Builds all Telegram keyboards for the bot.
✅ NEW: Implements the unified Exit Management control panel and sub-panels.
✅ FIXED: Restored build_subscription_keyboard for compatibility with auth module.
...
class NavigationBuilder:
    ...
    def build_pagination(current_page: int, total_pages: int) -> List[List[InlineKeyboardButton]]:
        ...
        if current_page > 1:
            buttons.append(
                InlineKeyboardButton(ButtonTexts.PREVIOUS, callback_data=CallbackBuilder.create(CallbackNamespace.NAVIGATION, CallbackAction.NAVIGATE, current_page - 1))
            )
        if total_pages > 1:
            buttons.append(InlineKeyboardButton(f"{current_page}/{total_pages}", callback_data="noop"))
        if current_page < total_pages:
            buttons.append(
                InlineKeyboardButton(ButtonTexts.NEXT, callback_data=CallbackBuilder.create(CallbackNamespace.NAVIGATION, CallbackAction.NAVIGATE, current_page + 1))
            )
...
async def build_open_recs_keyboard(items: List[Any], current_page: int, price_service: PriceService) -> InlineKeyboardMarkup:
    try:
        ...
        prices_map = {
            _get_attr(item, 'asset'): await price_service.get_cached_price(
...
# --- FIXED: Added missing keyboard for auth / join channel ---
def build_subscription_keyboard(channel_link: Optional[str]) -> Optional[InlineKeyboardMarkup]:
    """Builds a simple join-channel button for authentication or onboarding."""
    if channel_link:
        return InlineKeyboardMarkup([
