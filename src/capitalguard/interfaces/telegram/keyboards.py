# src/capitalguard/interfaces/telegram/keyboards.py (v21.4 - Production Ready & Final)
"""
...
✅ بنية CallbackBuilder موحدة لضمان الاتساق ومنع تجمد الأزرار.
✅ إضافة لوحة مفاتيح تأكيد عامة قابلة لإعادة الاستخدام.
✅ بنية نهائية ومستقرة.
"""
...
class CallbackBuilder:
    ...
    def create(namespace: Union[CallbackNamespace, str], action: Union[CallbackAction, str], *params) -> str:
        """Creates a new callback data string in a safe and unified manner."""
    ...
    def parse(callback_data: str) -> Dict[str, Any]:
        """Parses a callback data string into its components."""
        try:
...
async def build_open_recs_keyboard(items: List[Any], current_page: int, price_service: PriceService) -> InlineKeyboardMarkup:
    """Builds the keyboard for open positions with pagination."""
    try:
...
def build_confirmation_keyboard(namespace: str, item_id: int, confirm_text: str = "✅ Confirm", cancel_text: str = "❌ Cancel") -> InlineKeyboardMarkup:
    """Builds a generic confirmation keyboard."""
    return InlineKeyboardMarkup([[
...
__all__ = [
    ...
    'build_user_trade_control_keyboard', 'build_confirmation_keyboard',
