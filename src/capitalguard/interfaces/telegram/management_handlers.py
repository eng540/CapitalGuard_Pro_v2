...
async def unified_management_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, **kwargs):
    ...
    elif action == "edit_sl": await _prompt_for_input(query, context, "edit_sl", f"✏️ Reply with the new Stop Loss for #{rec_id}.")
    elif action == "edit_tp": await _prompt_for_input(query, context, "edit_tp", f"🎯 Reply with new targets for #{rec_id} (e.g., 50k 52k@50).")
    elif action == "close_manual": await _prompt_for_input(query, context, "close_manual", f"✍️ Reply with the closing price for #{rec_id}.")
