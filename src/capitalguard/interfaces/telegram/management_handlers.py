...
# src/capitalguard/interfaces/telegram/management_handlers.py (v30.15 - Indentation & Changelog)
"""
Handles all post-creation management of recommendations AND UserTrades.

CHANGELOG:
- v30.15:
    - Minor indentation cleanup and consistency pass across file.
    - Added this changelog header (requested).
    - Verified ConversationHandler registration flags and callback patterns.
    - Ensured safe state cleanup and consistent context.user_data keys.
- Prior fixes retained:
    - Imported _get_attr from helpers to resolve NameError in _send_or_edit_position_panel.
    - Restored submenus and conversations (partial close, exit strategy, user trade close).
    - Fixed callback parsing in user_trade_close_start to read trade id from params[1].
    - Added per_message=False to ConversationHandler registrations to suppress PTBUserWarning.
    - Added missing sqlalchemy import in repository (separate file).
"""
