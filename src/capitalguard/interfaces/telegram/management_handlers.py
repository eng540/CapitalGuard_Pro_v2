...
src/capitalguard/interfaces/telegram/management_handlers.py (v31.1 - CallbackQuery.from_dict Removal & Safe Cancel Hotfix)

""" Handles all post-creation management of recommendations via a unified UX. ✅ FIX: Removed use of CallbackQuery.from_dict and replaced with context.user_data-based state. ✅ FIX: prompt_handler now stores chat_id, message_id, and original_message_text for safe edits. ✅ FIX: cancel_input_handler and cancel_all_handler no longer mutate CallbackQuery.data; they create a lightweight fake callback_query object to simulate callback re-entry safely. ✅ FIX: Preserved all original flows, validations, and UX. No functionality removed. ✅ UX: Added Cancel button during input prompts and confirmation flow preserved. ✅ ROBUSTNESS: Compatible with python-telegram-bot v20+ and Railway deployments. This is the final, complete, and production-ready version. """

import logging import time from decimal import Decimal from typing import Optional, Dict, Any, Union

from telegram import Update, ReplyKeyboardRemove, CallbackQuery, InlineKeyboardButton, InlineKeyboardMarkup from telegram.constants import ParseMode from telegram.error import BadRequest, TelegramError from telegram.ext import ( Application, CallbackQueryHandler, MessageHandler, ContextTypes, filters, ConversationHandler, CommandHandler )

from capitalguard.infrastructure.db.uow import uow_transaction from .helpers import get_service, parse_cq_parts from .keyboards import ( analyst_control_panel_keyboard, build_open_recs_keyboard, build_user_trade_control_keyboard, build_close_options_keyboard, build_trade_data_edit_keyboard, build_exit_management_keyboard, build_partial_close_keyboard, CallbackAction, CallbackNamespace, build_confirmation_keyboard, CallbackBuilder, ButtonTexts ) from .ui_texts import build_trade_card_text from .auth import require_active_user, require_analyst_user from .parsers import parse_number, parse_targets_list, parse_trailing_distance from capitalguard.application.services.trade_service import TradeService from capitalguard.application.services.price_service import PriceService from capitalguard.domain.entities import RecommendationStatus

log = logging.getLogger(name) loge = logging.getLogger("capitalguard.errors")

--- Constants ---

AWAITING_INPUT_KEY = "awaiting_management_input" # Stores {"namespace", "action", "rec_id", "original_query", "previous_callback", "chat_id", "message_id", "original_message_text"} PENDING_CHANGE_KEY = "pending_management_change" # Stores validated value before confirmation LAST_ACTIVITY_KEY = "last_activity_management" MANAGEMENT_TIMEOUT = 1800 # 30 minutes

--- Session & Timeout Management ---

def init_management_session(context: ContextTypes.DEFAULT_TYPE): context.user_data[LAST_ACTIVITY_KEY] = time.time() context.user_data.pop(AWAITING_INPUT_KEY, None) context.user_data.pop(PENDING_CHANGE_KEY, None) log.debug(f"Management session initialized/reset for user {getattr(context, '_user_id', 'unknown')}.")

def update_management_activity(context: ContextTypes.DEFAULT_TYPE): context.user_data[LAST_ACTIVITY_KEY] = time.time()

def clean_management_state(context: ContextTypes.DEFAULT_TYPE): for key in [AWAITING_INPUT_KEY, LAST_ACTIVITY_KEY, PENDING_CHANGE_KEY]: context.user_data.pop(key, None)

async def handle_management_timeout(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool: if LAST_ACTIVITY_KEY not in context.user_data: # No session active, nothing to time out return False if time.time() - context.user_data.get(LAST_ACTIVITY_KEY, 0) > MANAGEMENT_TIMEOUT: clean_management_state(context) msg = "⏰ انتهت مدة الجلسة بسبب عدم النشاط.\n\nيرجى استخدام /myportfolio للبدء من جديد." if update.callback_query: # Try to answer callback first, then edit message try: await update.callback_query.answer("انتهت مدة الجلسة", show_alert=True) except TelegramError: pass # Ignore if callback expired # Use context to get message reference if query is gone try: message_to_edit = update.callback_query.message await safe_edit_message(update.callback_query, message=message_to_edit, text=msg, reply_markup=None) except Exception: # fallback to sending a new message try: await update.callback_query.message.reply_text(msg) except Exception: pass elif update.message: await update.message.reply_text(msg) return True return False

--- Helper Functions ---

async def safe_edit_message(query: Optional[CallbackQuery], message=None, text: str = None, reply_markup=None, parse_mode: str = ParseMode.HTML) -> bool: """Edits a message safely, preferring query if available.""" target_message = message if query: target_message = query.message if not target_message: log.warning("safe_edit_message called without a valid message or query.") return False
...
try:
    ...
    if query and "message to edit not found" in str(e).lower():
         log.debug(f"Ignoring 'message not found' during edit for query {getattr(query, 'id', '?')}.")
         return False
...
async def _send_or_edit_position_panel(update: Update, context: ContextTypes.DEFAULT_TYPE, db_session, position_type: str, position_id: int): """Fetches position details and renders the appropriate control panel.""" query = update.callback_query # Prefer query for editing message_target = query.message if query else update.effective_message try: trade_service = get_service(context, "trade_service", TradeService) position = trade_service.get_position_details_for_user(db_session, str(update.effective_user.id), position_type, position_id) if not position: await safe_edit_message(query, message=message_target, text="❌ المركز غير موجود أو تم إغلاقه.", reply_markup=None) return
...
@uow_transaction @require_active_user async def management_entry_point_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, db_session, **kwargs): """Handles /myportfolio and /open commands.""" init_management_session(context) # Clean state before starting try: trade_service = get_service(context, "trade_service", TradeService) price_service = get_service(context, "price_service", PriceService) items = trade_service.get_open_positions_for_user(db_session, str(update.effective_user.id)) if not items: await update.message.reply_text("✅ لا توجد مراكز مفتوحة حالياً.") return keyboard = await build_open_recs_keyboard(items, current_page=1, price_service=price_service) await update.message.reply_html("<b>📊 المراكز المفتوحة</b>\nاختر مركزاً للإدارة:", reply_markup=keyboard) except Exception as e: loge.error(f"Error in management entry point: {e}", exc_info=True) await update.message.reply_text("❌ خطأ في تحميل المراكز المفتوحة.")

@uow_transaction @require_active_user async def navigate_open_positions_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, db_session, **kwargs): """Handles pagination for the open positions list.""" query = update.callback_query await query.answer() if await handle_management_timeout(update, context): return update_management_activity(context)
...
try:
    ...
    await safe_edit_message(query, message=query.message, text="<b>📊 المراكز المفتوحة</b>\nاختر مركزاً للإدارة:", reply_markup=keyboard)
except Exception as e:
    loge.error(f"Error in open positions navigation: {e}", exc_info=True)
    await safe_edit_message(query, message=query.message, text="❌ خطأ في تحميل المراكز.")

@uow_transaction @require_active_user async def show_position_panel_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, db_session, **kwargs): """Shows the detailed control panel for a selected position.""" query = update.callback_query await query.answer() if await handle_management_timeout(update, context): return update_management_activity(context) # Clear any pending input state when showing a panel context.user_data.pop(AWAITING_INPUT_KEY, None) context.user_data.pop(PENDING_CHANGE_KEY, None)
...
try:
    ...
    await safe_edit_message(query, message=query.message, text="❌ بيانات استدعاء غير صالحة.", reply_markup=None)

@uow_transaction @require_active_user @require_analyst_user # Only analysts can access submenus async def show_submenu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, db_session, db_user, **kwargs): """Displays specific submenus like Edit, Close, Partial Close, Exit Management.""" query = update.callback_query await query.answer() if await handle_management_timeout(update, context): return update_management_activity(context)
...
if not rec:
    ...
    await safe_edit_message(query, message=query.message, text="❌ التوصية غير موجودة أو تم إغلاقها.", reply_markup=None)
    return
...
if keyboard:
    await safe_edit_message(query, message=query.message, text=text, reply_markup=keyboard)
else:
...
async def prompt_handler(update: Update, context: ContextTypes.DEFAULT_TYPE): """Asks the user to send the new value as a reply.""" query = update.callback_query await query.answer() if await handle_management_timeout(update, context): return update_management_activity(context)
...
context.user_data[AWAITING_INPUT_KEY] = {
    ...
    "action": action,
    "rec_id": rec_id,
    "original_query": query.to_dict(), # Store query data for potential reuse (kept for debugging)
    "previous_callback": previous_callback_data, # Store where to go back on cancel
    "chat_id": query.message.chat.id,
    "message_id": query.message.message_id,
    "original_message_text": getattr(query.message, "text_html", getattr(query.message, "text", ""))
}
...
await safe_edit_message(query, message=query.message, text=f"{getattr(query.message, 'text_html', getattr(query.message, 'text', ''))}\n\n<b>{prompt_text}</b>", reply_markup=input_keyboard)

@uow_transaction @require_active_user @require_analyst_user # Modifications require analyst async def reply_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, db_session, db_user, **kwargs): """Handles the text reply containing the new value, validates it, and asks for confirmation.""" if await handle_management_timeout(update, context): return update_management_activity(context)
...
if not (state and update.message and update.message.text):
    log.debug("Reply handler ignored: No valid state or not a reply.")
    return

# Restore saved context from state
chat_id = state.get("chat_id")
message_id = state.get("message_id")
original_message_text = state.get("original_message_text", "")
...
try:
    ...
    if validated_value is not None or action == "edit_notes": # Explicitly allow None for edit_notes
        ...
        # Use bot.edit_message_text since we no longer reconstruct original CallbackQuery
        try:
            await context.bot.edit_message_text(
                chat_id=chat_id,
                message_id=message_id,
                text=f"❓ <b>تأكيد الإجراء</b>\n\nهل أنت متأكد أنك تريد:\n➡️ {change_description}؟",
                reply_markup=confirm_keyboard,
                parse_mode=ParseMode.HTML,
                disable_web_page_preview=True
            )
        except Exception as e:
            loge.error(f"Failed to edit confirmation message for rec #{rec_id}: {e}", exc_info=True)
            # Fallback to sending a new message
            await context.bot.send_message(chat_id=update.effective_chat.id, text=f"❓ تأكيد: {change_description}\n\n(تحذير: لم أستطع تعديل الرسالة الأصلية.)")
    else:
    ...
    # Use original_message_text saved in state for display
    base_text = original_message_text or ""
    try:
        await context.bot.edit_message_text(
            chat_id=chat_id,
            message_id=message_id,
            text=f"{base_text}\n\n⚠️ <b>خطأ:</b> {e}\n\nيرجى إعادة إدخال القيمة الصحيحة:",
            reply_markup=input_keyboard,
            parse_mode=ParseMode.HTML,
            disable_web_page_preview=True
        )
    except Exception:
        # Fallback to sending a message if edit failed
        await context.bot.send_message(chat_id=update.effective_chat.id, text=f"⚠️ خطأ: {e}\nأعد الإدخال.")
...
@uow_transaction @require_active_user @require_analyst_user async def confirm_change_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, db_session, db_user, **kwargs): """Executes the pending change after user confirmation.""" query = update.callback_query await query.answer("جاري التنفيذ...") if await handle_management_timeout(update, context): return
...
async def cancel_input_handler(update: Update, context: ContextTypes.DEFAULT_TYPE): """Handles cancellation during the text input phase by returning to the previous submenu.""" query = update.callback_query await query.answer() if await handle_management_timeout(update, context): return

state = context.user_data.pop(AWAITING_INPUT_KEY, None)
context.user_data.pop(PENDING_CHANGE_KEY, None) # Clean pending value too

# Create a lightweight fake callback query object to avoid mutating existing CallbackQuery
class _FakeCQ:
    def __init__(self, original_query_obj, data):
        self.data = data
        self.message = original_query_obj.message
        self.from_user = original_query_obj.from_user
        self.id = getattr(original_query_obj, "id", None)
...
if state and state.get("previous_callback"):
    prev_cb = state["previous_callback"]
    fake_cq = _FakeCQ(query, prev_cb)
    new_update = Update(update.update_id + 1, callback_query=fake_cq)
    # Need db_session for show_submenu_handler - wrap in uow and decorators
    try:
        await uow_transaction(require_active_user(require_analyst_user(show_submenu_handler)))(new_update, context)
    except Exception as e:
        loge.error(f"Failed to re-enter submenu on cancel_input_handler: {e}", exc_info=True)
        # Fallback: edit original message
        try:
            await safe_edit_message(query, message=query.message, text="❌ تم إلغاء الإدخال.")
        except Exception:
            pass
elif state:
     ...
     if rec_id:
         back_cb = CallbackBuilder.create(CallbackNamespace.POSITION, CallbackAction.SHOW, 'rec', rec_id)
         fake_cq = _FakeCQ(query, back_cb)
         new_update = Update(update.update_id + 1, callback_query=fake_cq)
         try:
             await uow_transaction(require_active_user(show_position_panel_handler))(new_update, context)
         except Exception as e:
             loge.error(f"Failed to open main panel in cancel_input_handler: {e}", exc_info=True)
             try:
                 await safe_edit_message(query, message=query.message, text="❌ تم إلغاء الإدخال.")
             except Exception:
                 pass
...
async def cancel_all_handler(update: Update, context: ContextTypes.DEFAULT_TYPE): """Handles cancellation during the confirmation phase by returning to the main panel.""" query = update.callback_query await query.answer() if await handle_management_timeout(update, context): return
...
# Create fake callback query for show_position_panel_handler
class _FakeCQ2:
    def __init__(self, original_query_obj, data):
        self.data = data
        self.message = original_query_obj.message
...
back_cb = CallbackBuilder.create(CallbackNamespace.POSITION, CallbackAction.SHOW, 'rec', rec_id)
fake_cq = _FakeCQ2(query, back_cb)
new_update = Update(update.update_id + 1, callback_query=fake_cq)
# Need db_session for show_position_panel_handler - wrap in uow
try:
    await uow_transaction(require_active_user(show_position_panel_handler))(new_update, context)
except Exception as e:
    loge.error(f"Failed in cancel_all_handler to restore panel: {e}", exc_info=True)
    try:
        await safe_edit_message(query, message=query.message, text="❌ تم إلغاء العملية.")
    except Exception:
        pass

--- Immediate Action Handlers ---

@uow_transaction @require_active_user @require_analyst_user # Most immediate actions are analyst-only async def immediate_action_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, db_session, db_user, **kwargs): """Handles actions that execute immediately without needing user text input.""" query = update.callback_query # Give immediate feedback that the button was received await query.answer("جاري التنفيذ...") if await handle_management_timeout(update, context): return update_management_activity(context)
...
@uow_transaction @require_active_user @require_analyst_user async def partial_close_fixed_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, db_session, db_user, **kwargs): """Handles partial close buttons with fixed percentages (e.g., 25%, 50%).""" query = update.callback_query await query.answer("جاري التنفيذ...") if await handle_management_timeout(update, context): return update_management_activity(context)
...
def register_management_handlers(app: Application): """Registers all management-related handlers.""" app.add_handler(CommandHandler(["myportfolio", "open"], management_entry_point_handler))
