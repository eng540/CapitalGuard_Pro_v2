...
# src/capitalguard/interfaces/telegram/management_handlers.py (v31.0 - R1 Task 6 UI Logic)
"""
Handles all post-creation management of recommendations AND UserTrades.
CHANGELOG:
- v31.0 (R1 Task 6):
    - ✅ THE FIX: Updated `management_entry_point_handler` (/myportfolio) to
      filter positions into `activated_items` and `watchlist_items` based
      on their status.
    - It now passes these two distinct lists to `build_open_recs_keyboard`
      to fulfill the UI requirement of separating portfolio layers.
- v30.16 (Cumulative):
    - Fixed `TypeError` in `_send_or_edit_position_panel` by passing `orm_status_value`.
...
from capitalguard.interfaces.telegram.keyboards import _format_price, _truncate_text
    ...
    try:
        ...
        for item in items:
            ...
            if is_trade:
                ...
                status_value = _get_attr(item, 'status').value
    ...
    try:
        ...
        if namespace == CallbackNamespace.EXIT_STRATEGY.value:
            if action == "set_fixed":
                ...
                entry_dec = _get_attr(current_item, 'entry')
                if (_get_attr(current_item, 'side') == "LONG" and price <= entry_dec) or (
                    _get_attr(current_item, 'side') == "SHORT" and price >= entry_dec
                ):
            ...
            if action in ["edit_sl", "edit_entry", "close_manual"]:
                ...
                if action == "edit_sl":
                    ...
                    trade_service._validate_recommendation_data(
                        _get_attr(current_item, 'side'),
                        _get_attr(current_item, 'entry'),
                        price,
                    ...
                    trade_service._validate_recommendation_data(
                        _get_attr(current_item, 'side'),
                        price,
                        _get_attr(current_item, 'stop_loss'),
                        current_item.targets.values,
                    )
                    validated_value = price
    ...
    try:
        ...
        if namespace == CallbackNamespace.EXIT_STRATEGY.value:
            ...
            if action == "edit_sl":
                ...
                live_price = await price_service.get_cached_price(_get_attr(rec, 'asset'), _get_attr(rec, "market"), force_refresh=True)
                if not live_price:
        ...
        if namespace == CallbackNamespace.EXIT_STRATEGY.value:
            ...
            if action == "close_market":
                ...
                try:
                    ...
                    live_price = await price_service.get_cached_price(_get_attr(position, 'asset'), _get_attr(position, "market"), force_refresh=True)
                    if not live_price:
                        raise ValueError(f"Could not fetch market price for {_get_attr(position, 'asset')}.")
...
async def user_trade_close_start(update: Update, context: ContextTypes.DEFAULT_TYPE, db_session, db_user, **kwargs) -> int:
    ...
    await safe_edit_message(
        ...
        query.message.chat_id,
        query.message.message_id,
        text=f"{query.message.text_html}\n\n<b>✍️ Send the final Exit Price for {_get_attr(position, 'asset')}:</b>",
        reply_markup=InlineKeyboardMarkup([[cancel_button]]),
    )
