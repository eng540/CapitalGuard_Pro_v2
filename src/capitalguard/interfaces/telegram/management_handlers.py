...
def _refresh_bucket_key(update: Update) -> Optional[str]:
    q = update.callback_query
    if not q or not q.message or not q.message.chat or q.message.message_id is None:
        return None
...
async def track_signal_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    rec_id = _parse_tail_int(query.data)
    if rec_id is None:
        ...
        return
    try:
        await query.answer("📌 تم استلام طلب التتبع — قريبًا ستُفعّل هذه الميزة.", show_alert=False)
    except Exception:
        pass
