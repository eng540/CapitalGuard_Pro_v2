...
# Version: v102.8.0-SCOPE-FIX (Critical Channel Fix)
# ✅ CRITICAL FIXES:
#    1. SCOPE RESTRICTION: Text inputs now ONLY accepted in Private Chats (Stops Channel Spam).
#    2. AUTH SAFETY: Prevents AttributeError by ensuring handler only runs where a user exists.
#    3. SILENT IGNORE: If no session exists, ignore text instead of throwing errors.
#    4. Uses Central Parsers for robust number handling.
...
class PortfolioController:
    ...
    async def handle_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE, db_session, db_user):
        ...
        # ✅ FIX: Silent return if no session (Avoids spamming channels/private chats)
        if not state:
            return
        ...
        if missing_fields:
            ...
            # Silent return on corruption too to be safe
            return
        
        text_val = update.message.text.strip()
        
        action = state.get("action")
        # Explicit cast to int to avoid DB lookup errors
        try:
            rec_id = int(state.get("rec_id"))
        except (ValueError, TypeError):
            return # Silent fail on invalid ID
        ...
        try:
            ...
            # --- TEXT INPUTS (Notes) ---
                ...
                return # Silent ignore on unknown action
...
class ActionRouter:
    ...
    _EDIT_ROUTES = {
        ...
        "add_notes": PortfolioController.handle_edit_selection
    }
    _SUBMENU_ROUTES = {
...
def register_management_handlers(app: Application):
    ...
    # ✅ CRITICAL FIX: Only accept text in PRIVATE chats to prevent channel spam
