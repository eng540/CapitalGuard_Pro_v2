# --- START OF FINAL, RE-ARCHITECTED, AND READY-TO-USE FILE ---
import re
import unicodedata
...
# ✅ --- دالة التحليل الجديدة والمحسّنة للأهداف ---
def parse_targets_list(tokens: List[str]) -> List[Dict[str, float]]:
    ...
    for token in tokens:
        ...
        if '@' in token:
            ...
            # هدف بسيط بدون نسبة إغلاق
            price = _parse_one_number(token)
            parsed_targets.append({"price": price, "close_percent": 0.0})
            
    # منطق افتراضي: إذا لم يحدد المستخدم أي نسب، نعتبر الهدف الأخير هو هدف الإغلاق الكامل
...
# --- دوال التحليل عالية المستوى (للأوامر والمحرر) ---

def parse_quick_command(text: str) -> Optional[Dict[str, Any]]:
    try:
        # استخدام تعبير نمطي أكثر مرونة للأهداف
        parts = text.strip().split()
        if len(parts) < 5 or parts[0].lower() != '/rec':
            return None
...
def parse_text_editor(text: str) -> Optional[Dict[str, Any]]:
    ...
    return data
# --- END OF FINAL, RE-ARCHITECTED, AND READY-TO-USE FILE ---