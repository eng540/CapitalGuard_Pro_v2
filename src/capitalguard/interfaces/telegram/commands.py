...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Handles the /start command. Greets new users and prompts them to register,
    ...
    if user:
        ...
        # New user, present the registration prompt with a button.
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("✅ الموافقة على الشروط وبدء المتابعة", callback_data="user_register_confirm")]
        ])
        welcome_text = (
            "👋 <b>أهلاً بك في CapitalGuard!</b>\n\n"
            "مساعدك الشخصي لتتبع صفقاتك وتحليلاتك بسلاسة.\n\n"
            "للبدء، يرجى الموافقة على شروط الخدمة وسياسة الخصوصية."
        )
        await update.message.reply_html(welcome_text, reply_markup=keyboard)


async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Displays a help message with available commands for registered users."""
    await update.message.reply_html(
        "<b>الأوامر المتاحة:</b>\n\n"
        "• <code>/newrec</code> — إنشاء توصية جديدة.\n"
        "• <code>/open [filter]</code> — عرض لوحة قيادة التوصيات المفتوحة.\n"
        "• <code>/stats</code> — عرض ملخّص الأداء الخاص بك.\n"
        "• <code>/export</code> — تصدير جميع توصياتك في ملف CSV.\n"
        "• <code>/settings</code> — تغيير طريقة الإدخال المفضلة لديك."
    )

# --- Other commands (open, stats, export) remain unchanged ---
# These are protected by the ALLOWED_USER_FILTER
...
async def stats_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    stats = analytics_service.performance_summary(); text = build_analyst_stats_text(stats)
...
async def export_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    output.seek(0); bytes_buffer = io.BytesIO(output.getvalue().encode('utf-8'))
...
def register_commands(app: Application):
    ...
    # Public command, everyone can start a conversation
    app.add_handler(CommandHandler("start", start_cmd))

    # Protected commands, only registered users can access
