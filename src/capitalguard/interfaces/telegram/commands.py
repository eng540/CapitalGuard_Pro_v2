# src/capitalguard/interfaces/telegram/commands.py (v25.7 - FINAL & DECORATOR-SAFE)
"""
Registers and implements all non-conversational commands for the Telegram bot.
This version applies decorators correctly to ensure proper authorization and DB session handling.
"""
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE, db_session, **kwargs):
    """Handles the /start command, including user creation and deep linking."""
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE, db_session, db_user, **kwargs):
    ...
    trader_help = "• <code>/myportfolio</code> — View your open trades.\n"
    analyst_help = (
        "• <code>/newrec</code> — Create a new recommendation.\n"
        "• <code>/channels</code> — View & manage your linked channels.\n"
    )
    general_help = "• <code>/help</code> — Show this help message."
