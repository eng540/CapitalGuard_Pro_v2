...
# Version: v12.0.0-INTEGRATED (Live Price Flow Fix)
# ✅ THE FIX:
#     1. Price Injection: Fetched price is attached to 'rec' object so sub-functions can see it.
#     2. Zero-Wait: If live price is missing, Entry price is used as fallback immediately.
#     3. Logic Flow: Ensures 'Dashboard' always has data to display.
...
def _get_target_icon(target_index: int, hit_targets: set, total_targets: int) -> str:
    if target_index in hit_targets: return ICON_HIT
    ...
    return ICON_NEXT if target_index == next_unhit else ICON_WAIT
...
def _build_status_dashboard(rec: Recommendation, is_initial_publish: bool = False) -> str:
    try:
        status = _get_attr(rec, 'status')
        # ✅ KEY FIX: Use the injected live_price from the object
...
def _build_targets_block(rec: Recommendation) -> str:
    try:
        ...
        for i, t in enumerate(t_list, 1):
            ...
            elif icon == ICON_NEXT: p_fmt = f"<b>{p_fmt}</b>"
...
async def build_trade_card_text(rec: Recommendation, bot_username: str, is_initial_publish: bool = False) -> str:
    """
    Generates the trade card.
    ✅ NOW: Ensures price is injected into the object.
    """
    try:
        symbol = _get_attr(rec.asset, 'value', 'SYMBOL')
        market = getattr(rec, 'market', 'Futures') or 'Futures'
        
        # 1. Get Price (if not already injected)
        cached_price = getattr(rec, 'live_price', None)
        if not cached_price:
            cached_price = await get_live_price(symbol, market)
            
        # 2. Force Fallback if still None (Use Entry)
        if not cached_price:
            cached_price = float(_to_decimal(_get_attr(rec, 'entry', 0)))
            
        # 3. INJECT into object so sub-functions can see it
