...
(I_ASSET_CHOICE, I_ASSET_NEW, I_SIDE_MARKET, I_ORDER_TYPE, I_PRICES, I_NOTES, I_REVIEW) = range(3, 10)
...
async def change_market_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Handles the 'Change Market' button press by showing market choices."""
    query = update.callback_query; await query.answer()
...
async def notes_received(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if review_key and review_key in context.bot_data and original_message:
        ...
        # Create a dummy update object to pass to show_review_card
        dummy_query = type('obj', (object,), {'message': original_message})
        dummy_update = type('obj', (object,), {'callback_query': dummy_query, 'message': None})
