# --- START OF FULL, FINAL, AND READY-TO-USE FILE: src/capitalguard/interfaces/telegram/conversation_handlers.py ---
import logging
import uuid
import types
...
# --- State Definitions ---
(SELECT_METHOD, AWAIT_QUICK_COMMAND, AWAIT_TEXT_EDITOR, I_ASSET, I_SIDE_MARKET, I_ORDER_TYPE, I_PRICES, I_NOTES, I_REVIEW) = range(9)
...
async def quick_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not data:
        await update.message.reply_text(
            "❌ صيغة غير صحيحة. يجب أن تبدأ الرسالة بـ `/rec` وتحتوي على جميع الحقول. حاول مرة أخرى."
        )
        return AWAIT_QUICK_COMMAND
...
async def text_editor_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not data:
        await update.message.reply_text(
            "❌ تعذّر تحليل النص. تأكد من وجود الحقول الأساسية وحاول مرة أخرى."
        )
        return AWAIT_TEXT_EDITOR
    context.user_data[CONVERSATION_DATA_KEY] = data
    return await show_review_card(update, context)
...
async def notes_received(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if review_key and review_key in context.bot_data and original_message:
        ...
        dummy_update = Update(
            update.update_id,
            callback_query=types.SimpleNamespace(message=original_message, data='')
        )
    ...
    try:
        ...
        if report.get("success"):
            ...
            await query.edit_message_text(
                f"⚠️ تم حفظ التوصية #{saved_rec.id}، ولكن فشل النشر (قد لا تكون هناك قنوات مرتبطة)."
            )
    except Exception as e:
...
async def unexpected_input_fallback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """
    Forgiving fallback:
    - If user sends /newrec, /new, /editor, or /rec in the middle of a flow, restart accordingly.
    ...
    if context.user_data.get(CONVERSATION_DATA_KEY) is not None or context.user_data.get('current_review_key'):
        ...
        if update.message:
            ...
            await update.callback_query.answer("إجراء غير صالح. تم إنهاء المحادثة.", show_alert=True)
            try:
...
def register_conversation_handlers(app: Application):
    creation_conv_handler = ConversationHandler(
        ...
        states={
            ...
            # IMPORTANT: Do NOT combine with filters.TEXT here; /rec is a command.
            AWAIT_QUICK_COMMAND: [
                MessageHandler(filters.Regex(r'^\/rec'), quick_command_handler)
            ],
            AWAIT_TEXT_EDITOR: [
...
# --- END OF FULL, FINAL, AND READY-TO-USE FILE: src/capitalguard/interfaces/telegram/conversation_handlers.py ---