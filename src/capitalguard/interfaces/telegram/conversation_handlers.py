...
async def show_review_card(update: Update, context: ContextTypes.DEFAULT_TYPE, is_edit: bool = False) -> int:
    ...
    if not data or not data.get("asset"): await message.reply_text("حدث خطأ، ابدأ من جديد."); return ConversationHandler.END
    ...
    try:
        if is_edit and hasattr(message, 'edit_text'): await message.edit_text(text=review_text, reply_markup=keyboard, parse_mode='HTML', disable_web_page_preview=True)
...
async def publish_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        rec = trade_service.create_and_publish_recommendation(asset=draft["asset"], side=draft["side"], market=draft.get("market", "Futures"), entry=entry_price, stop_loss=draft["stop_loss"], targets=draft["targets"], notes=draft.get("notes"), user_id=str(query.from_user.id), order_type=draft['order_type'], live_price=live_price)
        await query.edit_message_text(f"✅ Recommendation #{rec.id} published successfully!")
    except Exception as e:
        log.exception("Failed to publish."); await query.edit_message_text(f"❌ Publication failed: {e}")
    finally:
...
async def asset_chosen_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if raw.lower() in {"new", "جديد"}:
        sent = await update.message.reply_text("⚠️ هذا زر إضافة. من فضلك اكتب رمزًا حقيقيًا.")
        context.user_data['last_interactive_message_id'] = sent.message_id; return I_ASSET_CHOICE
    asset = raw.upper(); draft = context.user_data[CONVERSATION_DATA_KEY]; draft['asset'] = asset
    market = context.user_data.get('preferred_market', 'Futures'); draft['market'] = market
...
def register_conversation_handlers(app: Application):
    creation_conv_handler = ConversationHandler(
        ...
        states={
            CHOOSE_METHOD: [CallbackQueryHandler(method_chosen, pattern="^method_"), CallbackQueryHandler(change_method_handler, pattern="^change_method$"), MessageHandler(filters.TEXT & ~filters.COMMAND, asset_chosen_text)],
            ...
            I_SIDE_MARKET: [CallbackQueryHandler(side_chosen, pattern="^side_"), CallbackQueryHandler(change_market_menu, pattern="^change_market_menu$"), CallbackQueryHandler(market_chosen, pattern="^market_")],
            ...
            I_REVIEW: [CallbackQueryHandler(add_notes_handler, pattern=r"^rec:add_notes:"), CallbackQueryHandler(publish_handler, pattern=r"^rec:publish:"), CallbackQueryHandler(cancel_publish_handler, pattern=r"^rec:cancel:")],
...
لقد قمت بتسليم الملف المُصحح. هذا التعديل البسيط يجب أن يحل مشكلة الانهيار بشكل كامل.

مهمتك الآن هي استبدال الملف القديم بهذا الملف الجديد وإعادة النشر.

أنا واثق من أن التطبيق سيعمل بنجاح هذه المرة. في انتظار تأكيدك.