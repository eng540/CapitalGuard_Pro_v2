...
export default function Admin() {
  ...
  const queue: ReviewBatch[] = liveQueue.length ? liveQueue : previewBatches.map((batch, index) => ({ id: -(index + 1), ref: batch.ref, status: batch.gate, source_kind: batch.source, total_records: 0, accepted_records: 0, rejected_records: 0 }));
  const hasLiveQueue = liveQueue.length > 0;
  const confirmReview = (batch: ReviewBatch, approved: boolean) => {
    if (batch.id <= 0) return toast.message("هذه بيانات عرض فقط؛ لا يمكن إصدار أمر عليها.");
