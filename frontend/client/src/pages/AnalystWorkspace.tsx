...
export default function AnalystWorkspace() {
  ...
  const dashboard = trpc.capitalguard.analyst.dashboard.useQuery(undefined, { refetchInterval: 30_000, refetchOnWindowFocus: true });
  ...
  return <DashboardLayout><main dir="rtl" className="mx-auto max-w-5xl space-y-6"><header><p className="text-xs font-semibold uppercase tracking-[.2em] text-cyan-300">Analyst Desk · Guided Flow</p><h1 className="mt-2 text-3xl font-semibold">إنشاء توصية بخطوات واضحة</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">اتبع التسلسل من السوق إلى الأمر؛ Core هو مصدر السعر والتحقق والنشر.</p></header>
    <section className="rounded-3xl border border-violet-300/15 bg-violet-300/[.04] p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[.16em] text-violet-200">Signal Health</p><h2 className="mt-1 font-semibold">صحة السجل المنشور</h2></div>{dashboard.data?.health.eligible_for_ranking ? <span className="rounded-full bg-emerald-300/10 px-3 py-1 text-xs text-emerald-200">مؤهل للترتيب</span> : <span className="rounded-full bg-amber-300/10 px-3 py-1 text-xs text-amber-200">عينة غير كافية</span>}</div>{dashboard.isLoading ? <p className="mt-3 text-sm text-muted-foreground">جارٍ تحميل مؤشرات Core…</p> : dashboard.isError ? <p className="mt-3 text-sm text-amber-100">تعذر تحميل مؤشرات الصحة؛ لا تؤثر على إنشاء المعاينة.</p> : dashboard.data ? <div className="mt-4 grid gap-3 sm:grid-cols-3 lg:grid-cols-6"><HealthMetric label="Sample" value={String(dashboard.data.health.sample_size)}/><HealthMetric label="Win Rate" value={`${dashboard.data.health.win_rate_pct.toFixed(1)}%`}/><HealthMetric label="PnL" value={`${dashboard.data.health.total_pnl_pct.toFixed(2)}%`}/><HealthMetric label="PF" value={dashboard.data.health.profit_factor_infinite ? "∞" : dashboard.data.health.profit_factor?.toFixed(2) ?? "—"}/><HealthMetric label="Drawdown" value={`${dashboard.data.health.max_drawdown_pct.toFixed(2)}%`}/><HealthMetric label="Exposure" value={`${dashboard.data.health.risk_exposure_pct.toFixed(2)}%`}/><HealthMetric label="Avg TP1" value={dashboard.data.health.signal_health.avg_minutes_to_first_target == null ? "—" : `${dashboard.data.health.signal_health.avg_minutes_to_first_target.toFixed(0)}m`}/><HealthMetric label="Reversed" value={String(dashboard.data.health.signal_health.reversed_before_entry_count)}/></div> : null}</section>
...
function HealthMetric({ label, value }: { label: string; value: string }) { return <div className="rounded-xl border border-white/8 bg-white/[.04] p-3"><p className="text-[10px] text-muted-foreground">{label}</p><p className="mt-1 font-mono text-sm text-violet-100">{value}</p></div>; }
