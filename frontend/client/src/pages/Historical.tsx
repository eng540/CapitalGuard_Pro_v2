...
function IntakeBatchView({ batch, report, onRefresh }: { batch: { id: number; ref: string; status: string; source_kind: string; total_records: number; accepted_records: number; rejected_records: number; created_at: string | null; items: Array<{ id: number; order: number | null; item_key: string | null; status: string; semantic_status: string; parse_status: string | null; source_verification: string; raw_text: string | null; missing_fields: string[]; conflicting_fields: string[]; canonical: Record<string, unknown>; rejection_reason: string | null }> }; report?: { counts: Record<string, number>; readiness: Record<string, boolean>; next_action: string }; onRefresh: () => void }) {
  return <div className="mt-5 rounded-2xl border border-white/10 bg-slate-950/40 p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-mono text-xs text-cyan-200">{batch.ref}</p><p className="mt-1 text-sm text-muted-foreground">{batch.total_records} عنصر · قابل للمراجعة {batch.accepted_records} · مرفوض {batch.rejected_records}</p></div><div className="flex items-center gap-2"><StatusPill value={batch.status}/><Button type="button" variant="outline" size="sm" onClick={onRefresh}><RefreshCw className="ml-2 h-3.5 w-3.5"/>تحديث</Button></div></div><div className="mt-4 space-y-2">{batch.items.map(item => <div key={item.id} className="rounded-xl border border-white/8 bg-white/[.025] p-3"><div className="flex flex-wrap items-center justify-between gap-2"><span className="font-mono text-xs text-violet-200">#{item.order ?? "—"} · {item.item_key ?? `item-${item.id}`}</span><div className="flex flex-wrap gap-2"><StatusPill value={item.status}/><StatusPill value={item.semantic_status}/><StatusPill value={item.source_verification}/></div></div><p className="mt-2 line-clamp-2 text-xs text-muted-foreground">{item.raw_text || "بدون نص؛ راجع بيانات الوسائط والمصدر."}</p>{item.missing_fields.length > 0 ? <p className="mt-2 text-xs text-amber-200">حقول ناقصة: {item.missing_fields.join("، ")}</p> : null}{item.conflicting_fields.length > 0 ? <p className="mt-2 text-xs text-rose-200">تعارض: {item.conflicting_fields.join("، ")}</p> : null}</div>)}</div>{report ? <div className="mt-4 rounded-xl border border-cyan-400/15 bg-cyan-400/[.035] p-3"><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-xs font-medium text-cyan-100">تقرير الدفعة</p><span className="rounded-lg bg-cyan-400/10 px-2 py-1 text-[11px] text-cyan-100">الخطوة التالية: {report.next_action}</span></div><div className="mt-3 grid gap-2 text-xs text-muted-foreground sm:grid-cols-4"><span>Evidence: {report.counts.evidence_records ?? 0}</span><span>Historical Signals: {report.counts.historical_signals ?? 0}</span><span>Replay Events: {report.counts.replay_events ?? 0}</span><span>Verified: {report.counts.verified_replay_events ?? 0}</span></div></div> : null}</div>;
}
