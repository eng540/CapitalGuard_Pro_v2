...
function normalizeSignal(signal: Record<string, unknown>) { return { public_ref: String(signal.public_ref ?? "signal"), asset: signal.asset == null ? null : String(signal.asset), side: signal.side == null ? null : String(signal.side), status: String(signal.status ?? "UNKNOWN"), confidence_score: signal.confidence_score == null ? null : String(signal.confidence_score), events: Number(signal.events ?? 0), verified_events: Number(signal.verified_events ?? 0), lifecycle_status: String(signal.lifecycle_status ?? "NOT_ACTIVATED"), last_event: signal.last_event == null ? null : String(signal.last_event), timeline: Array.isArray(signal.timeline) ? signal.timeline as TimelinePoint[] : [] }; }
